...
def dyadic_filter(signal, decimation_factor, shifted=0):
    #signal has 3 dimensions
    if shifted==1:
      filtered_signal = signal[:,:,1::decimation_factor] #shifted by one see above
    else:
      filtered_signal = signal[:,:,::decimation_factor]  #standard way
...
class DataGenerator(keras.utils.Sequence):  # on   dragon2 (tested)    #DataGenerator('train', train_idx, Y_train, 32, vec_len, shuffle=True)
  ...
  def __getitem__(self, index):
    ...
    #in this case each second one is shifted by 1    
    batch_x = [X_batch, dyadic_filter(X_batch, 2, shifted = 1), dyadic_filter(X_batch, 4, shifted = 1), dyadic_filter(X_batch, 8, shifted = 1), dyadic_filter(X_batch, 16, shifted = 1), 
               dyadic_filter(X_batch, 32, shifted = 1)]
...
'''
print(' ')
print('generator output x type: ', type(x) )

print('generator output x[0] shape: ', np.shape(x[0]) )
print(' ')

print(' ')
print('generator output y type: ', type(y) )

print('generator output y shape: ', np.shape(y) )
print(' ')
'''
...
x = Dropout(dr)(x)
x = Convolution2D(40, (1, 4), padding='valid', activation="relu", kernel_initializer='glorot_uniform')(x)
x = Dropout(dr)(x)
out_5 = Flatten()(x)

channel_6 = Input(shape=(2,vec_len//32, 1), name="input6")
x = Convolution2D(30, (2, 8), padding='valid', activation="relu", kernel_initializer='glorot_uniform')(channel_6)
x = Dropout(dr)(x)
x = Convolution2D(10, (1, 4), padding='valid', activation="relu", kernel_initializer='glorot_uniform')(x)
x = Dropout(dr)(x)
out_6 = Flatten()(x)
...
model.compile(loss='categorical_crossentropy', optimizer=optim, metrics=["accuracy", "categorical_accuracy"])
...
filepath = './weights_folder/fusion_shifted_4096_all_2'
...
for x in range(folds):
  ...
  print('score: ', score)
  print('Accuracies: ', mean(scores), std(scores))


print('Final Accuracy: ', mean(scores), std(scores))
