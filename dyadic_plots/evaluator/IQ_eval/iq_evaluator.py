...
class DataGenerator(tf.keras.utils.Sequence):  # on   dragon2 (tested)    #DataGenerator('train', train_idx, Y_train, 32, vec_len, shuffle=True)
  ...
  def __getitem__(self, index):
    ...
    #batch_x = X_batch
    batch_x = normalize_per_waveform(X_batch)  #normalized per waveform between -1 and 1
...
filepath = './weights_folder/iq_eval_4096_norm.keras'
...
plt.savefig('epochs_accuracy_iq_.png', bbox_inches='tight')
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig('epochs_loss_iq4.png', bbox_inches='tight')
...
def snr_conf(test_idx, y, snr, lb, ub):
    ...
    #plot_confusion_matrix(confnorm, labels=classes, title="Dyadic Confusion Matrix (SNR=%d)"%(snr))
    plot_confusion_matrix(confnorm, labels=classes, title="IQ Confusion Matrix Function (SNR=%d-%d)"%(lb, ub), fig="IQ_Confusion_Matrix_Function_(SNR=%d-%d)4.png"%(lb, ub) )  
...
plt.savefig('iq_snr_curve.png', bbox_inches='tight')
...
plt.ylabel("Classification Accuracy", fontsize=30)
plt.title("IQ Classification Accuracy ", fontsize=30)
plt.savefig('iq_snr_curve4.png', bbox_inches='tight')
